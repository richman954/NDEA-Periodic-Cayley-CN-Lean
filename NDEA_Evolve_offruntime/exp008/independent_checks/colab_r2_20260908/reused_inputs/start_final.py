...
root=Path('/content/exp008_check');source=root/'final_source'
...
with tarfile.open(archive) as tar:
    for item in tar.getmembers():
        ...
        if not item.isfile() or p.is_absolute() or '..' in p.parts or item.name in files:
...
cmd=[sys.executable,'-u','-B',str(source/'verify_combined.py'),'--root',str(source),
     '--lean',str(root/'lean-4.31.0-linux/bin/lean'),'--mathlib',str(root/'mathlib'),
     '--other-packages',str(root/'mathlib/.lake/packages'),'--output',str(root/'final_verification')]
