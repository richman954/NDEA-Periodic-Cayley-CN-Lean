...
root=pathlib.Path('/content/exp008_check')
inp=root/'inputs'
...
def clone(pin,target):
    ...
    assert run('pin_'+pin['name'],['git','-C',target,'rev-parse','HEAD']).strip()==pin['rev']
try:
    hashes=json.loads((inp/'INPUT_HASHES.json').read_text())
    assert all(sha(inp/k)==v for k,v in hashes.items())
    ...
    def build(module):
        ...
        built.add(module)
    build('Cache.Main')
    modules=set(re.findall(r'^import (Mathlib\.\S+)',(inp/'Exp007Foundation.lean').read_text(),re.M))
