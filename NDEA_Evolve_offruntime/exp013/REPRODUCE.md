...
The combined source in `lean/Exp013Combined.lean` contains the frozen
...
python3 -B verify_combined.py \
  --root /path/to/exp013-working-copy \
...
## Preserved first attempt and corrected verification

The original combined attempt failed because an inherited `Matrix` namespace
opening made the control's unqualified `smul_apply` reference ambiguous. The
corrected source uses `_root_.smul_apply`; the mathematical formulas and public
catalog are unchanged. Original source, logs, deliveries and the pending source
review are retained under `attempts/r1/`. They are not accepted final evidence.

The corrected source is checked again locally and on the distinct fresh
`exp013-independent-check-r2` VM. Its allocation history includes the first
Experiment 013 VM, and the receiver rejects reuse of that VM or its boot ID.
...
fixed paths under `/content/exp013_check` and refuse existing work directories;
...
## Formal controls and packet checks

The generic energy and uniqueness milestone is checked through its Lean source and exact
controls. There is no new floating-point uniqueness diagnostic. Individual
module checks use `run_lean.py` with the preserved local dependency paths;
the independent combined check deliberately excludes project artifacts.
...
The new session is `exp013-independent-check-r2`; initial environment and VM
identity records are retained under `remote_check/`. No Drive mount is used.
The user selected local Chromebook backups; the sealed packet and a fresh
recovery copy are preserved locally after qualification.
