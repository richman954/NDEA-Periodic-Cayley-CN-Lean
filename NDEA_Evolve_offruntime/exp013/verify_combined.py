...
record={'passed':False,'start_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),
        'runner_sha256':sha(Path(__file__)),'scope':'All project source re-elaborated using compatible external artifacts from pinned source checkouts, with recorded hashes. Compiler and library artifacts remain trusted inputs; this check does not rebuild them or prove source-to-artifact correspondence.'}
...
try:
    ...
    source=root/'lean/Exp013Combined.lean'
    ...
    project_modules={Path(name).stem for name in inputs['source_hashes']}|{'Exp013Combined','SuperpositionClosure'}
