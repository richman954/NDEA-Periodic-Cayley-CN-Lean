...
with tempfile.TemporaryDirectory(prefix='exp013_protocol_') as temp:
    fixture=Path(temp)
    ...
    shutil.copyfile(root/'lean/Exp012Foundation.lean',fixture/'lean/Exp012Foundation.lean')
    modules=['GenericClassical','GenericEnergy','GenericUniqueness','LegacyBridge','Controls']
    for name in modules:
    ...
    generator=load(fixture/'make_combined.py','exp013_generator_fixture')
    ...
    source=fixture/'lean/GenericClassical.lean';original=source.read_bytes()
    source.write_bytes(original.replace(b'import ClassicalUniqueness',b'import UnembeddedProject'))
    reject('unembedded_project_import_rejected',lambda:generator.build(fixture));source.write_bytes(original)
    source.write_bytes(original.replace(b'fixture_GenericClassical',b'fixture_Controls'))
    reject('duplicate_public_name_rejected',lambda:generator.build(fixture));source.write_bytes(original)
    foundation=fixture/'lean/Exp012Foundation.lean';frozen=foundation.read_bytes()
    ...
    (fixture/'lean/Exp013Combined.lean').write_text(combined)
    ...
    # fixture. This exercises the receiver schema, not Experiment 013 mathematics.
    previous=root.parent/'exp012/remote_check/downloaded_evidence'
    files={str(p.relative_to(previous)):p.read_bytes().replace(b'/content/exp012_check',b'/content/exp013_check')
        .replace(b'/content/exp012_bootstrap.py',b'/content/exp013_bootstrap.py')
        for p in previous.rglob('*') if p.is_file()}
    environment=json.loads(files['ENVIRONMENT.json'])
    environment.update(session='exp013-independent-check-r2',boot_id='00000000-0000-4000-8000-000000000013')
    ...
    allocation={'utc':environment['utc'],'root':'/content/exp013_check','session':'exp013-independent-check-r2',
        'vm_id':'SYNTHETIC_EXP013_FIXTURE','distinct_allocation':True,'previous_vm_ids':
        ['m-s-kkb-usw4c1-1loxz56gsupls','m-s-kkb-usw3b1-3oqo9i5fec2ow','m-s-kkb-use1c1-1e742c6fpiifs','m-s-kkb-usc1a1-d4w1b6hb9jym','m-s-kkb-usc1b1-2yftnii3g1zpd','m-s-kkb-usc1c1-1p1nzzki4492q','m-s-kkb-usc1c0-1k0px308tu45q']}
    ...
    bootstrap['cache_modules']=sorted(set(bootstrap['cache_modules'])|{'Mathlib.Analysis.InnerProductSpace.Adjoint'})
    ...
    for row in bootstrap['commands']:
        key=str(PurePosixPath(row['log']).relative_to('/content/exp013_check'))
...
record={'passed':True,'utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),
    ...
    'scope':'Bounded offline infrastructure exercises with synthetic and transformed predecessor fixtures. No Experiment 013 Lean proof or network action is performed or accepted by these tests.'}
