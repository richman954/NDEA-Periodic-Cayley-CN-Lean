...
with tempfile.TemporaryDirectory(prefix='exp011_protocol_') as temp:
    fixture=Path(temp)
    ...
    shutil.copyfile(root/'lean/Exp010Foundation.lean',fixture/'lean/Exp010Foundation.lean')
    modules=['SolutionLipschitz','Reconstruction','ScheduleBounds','UniformConvergence','Controls']
    for name in modules:
    ...
    generator=load(fixture/'make_combined.py','exp011_generator_fixture')
    combined,inputs=generator.build(fixture)
    require(len(inputs['expected_audits'])==5 and not any('phantom' in x for x in inputs['expected_audits']),
    ...
    source=fixture/'lean/SolutionLipschitz.lean';original=source.read_bytes()
    ...
    source.write_bytes(original.replace(b'fixture_SolutionLipschitz',b'fixture_Controls'))
    reject('duplicate_public_name_rejected',lambda:generator.build(fixture));source.write_bytes(original)
    foundation=fixture/'lean/Exp010Foundation.lean';frozen=foundation.read_bytes()
    ...
    (fixture/'lean/Exp011Combined.lean').write_text(combined)
    ...
    # fixture. This exercises the receiver schema, not Experiment 011 mathematics.
    previous=root.parent/'exp010/remote_check/downloaded_evidence'
    files={str(p.relative_to(previous)):p.read_bytes().replace(b'/content/exp010_check',b'/content/exp011_check')
        .replace(b'/content/exp010_bootstrap.py',b'/content/exp011_bootstrap.py')
        for p in previous.rglob('*') if p.is_file()}
    environment=json.loads(files['ENVIRONMENT.json'])
    environment.update(session='exp011-independent-check',boot_id='00000000-0000-4000-8000-000000000011')
    ...
    allocation={'utc':environment['utc'],'root':'/content/exp011_check','session':'exp011-independent-check',
        'vm_id':'SYNTHETIC_EXP010_FIXTURE','distinct_allocation':True,'previous_vm_ids':
        ['m-s-kkb-use1c1-1e742c6fpiifs','m-s-kkb-usc1a1-d4w1b6hb9jym','m-s-kkb-usc1b1-2yftnii3g1zpd','m-s-kkb-usc1c1-1p1nzzki4492q']}
    ...
    for row in bootstrap['commands']:
        key=str(PurePosixPath(row['log']).relative_to('/content/exp011_check'))
...
record={'passed':True,'utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),
    ...
    'scope':'Bounded offline infrastructure exercises with synthetic and transformed predecessor fixtures. No Experiment 011 Lean proof or network action is performed or accepted by these tests.'}
