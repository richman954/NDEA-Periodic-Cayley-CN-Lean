...
The combined source in `lean/Exp011Combined.lean` contains the frozen
...
python3 -B verify_combined.py \
  --root /path/to/exp011-working-copy \
...
fixed paths under `/content/exp011_check` and refuse existing work directories;
...
In the working copy, `python3 -B numerical_checks.py` reruns the off-grid
reconstruction diagnostics and records its script hash. The
[diagnostics report](NUMERICAL_DIAGNOSTICS.md) describes the dense/modal
comparisons, exact index selection, and analytic-tail qualifications.
