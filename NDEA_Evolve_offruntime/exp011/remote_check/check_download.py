...
def validate_fresh_bootstrap(files, root, final_inputs, final_result):
    ...
    remote_root = PurePosixPath('/content/exp011_check')
    ...
    require(sha(predecessor_environment_path.read_bytes()) ==
        'b6bb37230a1276e20273fc413a3aa5b8f1acbce633bdf6bbc2056682ea3e7417',
        'Accepted Experiment 010 environment record changed')
    ...
    require(environment.get('session') == allocation.get('session') == 'exp011-independent-check'
    ...
    require(isinstance(prior_vm_ids, list) and len(prior_vm_ids) == len(set(prior_vm_ids))
        and {'m-s-kkb-use1c1-1e742c6fpiifs','m-s-kkb-usc1a1-d4w1b6hb9jym',
    ...
    require(isinstance(launch_command, list) and len(launch_command) == 4 and
        ...
        and launch_command[1:] == ['-u', '-B', '/content/exp011_bootstrap.py'] and
...
require(remote['command']==['/content/exp011_check/lean-4.31.0-linux/bin/lean','-j','1',
    '/content/exp011_check/final_source/lean/Exp011Combined.lean'],'Unexpected remote command')
require(remote['lean_path']=='/content/exp011_check/final_verification/dependencies:'
    '/content/exp011_check/lean-4.31.0-linux/lib/lean','Unexpected remote import path')
