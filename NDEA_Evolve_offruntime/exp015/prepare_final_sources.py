...
if (root/'remote_check/FINAL_UPLOAD.json').exists():
    raise RuntimeError('Final source delivery receipt already exists; preserve it before preparing again')
inputs=json.loads((root/'evidence/FINAL_INPUTS.json').read_text())
sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
spec=importlib.util.spec_from_file_location('exp015_final_source_generator',root/'make_combined.py')
generator=importlib.util.module_from_spec(spec);spec.loader.exec_module(generator)
rebuilt,reconstructed=generator.build(root)
if reconstructed!=inputs or rebuilt!=(root/'lean/Exp015Combined.lean').read_text():
...
paths=set(inputs['source_hashes'])|{'lean/Exp015Combined.lean','evidence/FINAL_INPUTS.json',
...
request={'archive':'/content/exp015_check/'+destination.name,'sha256':sha(destination),
