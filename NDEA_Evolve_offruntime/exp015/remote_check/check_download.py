...
def read_evidence_archive(archive, expected_sha256):
    """Reject corrupt, unsafe, duplicate or uncovered evidence before any extraction."""
    require(archive.stat().st_size <= 1 << 30, 'Evidence archive exceeds size limit')
    require(sha(archive.read_bytes()) == expected_sha256, 'Archive checksum mismatch')
    files = {}
    total = 0
    with tarfile.open(archive) as tar:
        for item in tar:
            ...
            require(item.isfile() and not path.is_absolute() and '..' not in path.parts
                and '\\' not in item.name and '\x00' not in item.name
                and str(path) == item.name and item.name not in files, 'Unsafe archive member')
            total += item.size
            require(len(files) < 10000 and total <= 1 << 30, 'Evidence payload exceeds size limit')
            files[item.name] = tar.extractfile(item).read()
    manifest = json.loads(files['EVIDENCE_SHA256.json'])
    require(isinstance(manifest, dict)
        and set(manifest) == set(files)-{'EVIDENCE_SHA256.json'}, 'Incomplete evidence manifest')
    for name, digest in manifest.items():
        require(sha(files[name]) == digest, 'Evidence changed: '+name)
...
def validate_fresh_bootstrap(files, root, final_inputs, final_result):
    ...
    remote_root = PurePosixPath('/content/exp015_check')
    ...
    require(sha(predecessor_environment_path.read_bytes()) ==
        'afcb4b607059a972cc72a9bdf98d2992f917f9dd5a5ddc66bffb999ba1111e9b',
        'Accepted Experiment 014 environment record changed')
    ...
    require(environment.get('session') == allocation.get('session') == 'exp015-independent-check'
    ...
    require(isinstance(prior_vm_ids, list) and len(prior_vm_ids) == len(set(prior_vm_ids))
        and {'m-s-kkb-usw4c1-1loxz56gsupls','m-s-kkb-usw3b1-3oqo9i5fec2ow','m-s-kkb-use1c1-1e742c6fpiifs','m-s-kkb-usc1a1-d4w1b6hb9jym',
             ...
             'm-s-kkb-usc1c0-1k0px308tu45q',
             'm-s-kkb-use1c2-3vyfjmsvu7xz8',
             'm-s-kkb-use4c0-3g3ru54zvgn9o'} <= set(prior_vm_ids)
    ...
    require(isinstance(boot_id, str) and str(uuid.UUID(boot_id)) == boot_id and
        boot_id not in {predecessor_environment['boot_id'],
            '75274d51-536c-4a5f-a2de-f5814d0d098f',
            'f7323458-d65c-4ed0-9889-e70929f24694'}, 'Fresh boot identity did not change')
    ...
    require(isinstance(launch_command, list) and len(launch_command) == 4 and
        ...
        and launch_command[1:] == ['-u', '-B', '/content/exp015_bootstrap.py'] and
...
files,manifest=read_evidence_archive(a.archive,a.sha256)
...
require(remote['command']==['/content/exp015_check/lean-4.31.0-linux/bin/lean','-j','1',
    '/content/exp015_check/final_source/lean/Exp015Combined.lean'],'Unexpected remote command')
require(remote['lean_path']=='/content/exp015_check/final_verification/dependencies:'
    '/content/exp015_check/lean-4.31.0-linux/lib/lean','Unexpected remote import path')
