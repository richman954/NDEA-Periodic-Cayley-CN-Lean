...
env=dict(os.environ)
env['LEAN_PATH']=':'.join([str(root/'build/lib/lean'),str(root.parent/'exp008/build/lib/lean'),str(root.parent/'exp006/build/lib/lean')]+[str(p/'.lake/build/lib/lean') for p in packages.iterdir() if p.is_dir()]+[str(lean.parent.parent/'lib/lean')])
if source.stem=='Exp009Combined':
