"""Record the replacement fresh Colab allocation before installing proof inputs."""
...
record={'passed':True,'fresh_root':True,'prior_project_paths':prior,
    ...
    'session':'exp009-independent-check','root':str(root),'platform':platform.platform(),
    'python':platform.python_version(),'logical_cpus':os.cpu_count(),
    'boot_id':Path('/proc/sys/kernel/random/boot_id').read_text().strip(),
    'scope':'Replacement new Colab allocation. Project runtime paths absent before initialization. Pinned compiler and external libraries will be independently downloaded; no project artifacts uploaded.'}
