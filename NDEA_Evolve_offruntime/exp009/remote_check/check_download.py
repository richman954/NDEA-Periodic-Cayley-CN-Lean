...
def validate_fresh_bootstrap(files, root, final_inputs, final_result):
    ...
    remote_root = PurePosixPath('/content/exp009_check')
    ...
    lost_environment_path = root/'remote_check/lost_runtime/ENVIRONMENT_EXPECTED.json'
    accepted_local = {str(p.relative_to(root)) for p in [environment_path,
        allocation_path, bootstrap_inputs_path, bootstrap_script_path, lost_environment_path]}
    ...
    lost_environment = json.loads(lost_environment_path.read_text())
    ...
    require(environment.get('session') == allocation.get('session') == 'exp009-independent-check'
    ...
    require(isinstance(prior_vm_ids, list) and len(prior_vm_ids) == len(set(prior_vm_ids))
        and len(prior_vm_ids) >= 2 and all(isinstance(v, str) and v for v in prior_vm_ids)
    ...
    require(isinstance(boot_id, str) and str(uuid.UUID(boot_id)) == boot_id and
        boot_id != lost_environment['boot_id'], 'Fresh boot identity did not change')
    ...
    require(isinstance(launch_command, list) and len(launch_command) == 4 and
        ...
        and launch_command[1:] == ['-u', '-B', '/content/exp009_bootstrap.py'] and
...
require(remote['command']==['/content/exp009_check/lean-4.31.0-linux/bin/lean','-j','1',
    '/content/exp009_check/final_source/lean/Exp009Combined.lean'],'Unexpected remote command')
require(remote['lean_path']=='/content/exp009_check/final_verification/dependencies:'
    '/content/exp009_check/lean-4.31.0-linux/lib/lean','Unexpected remote import path')
