...
The combined source in `lean/Exp012Combined.lean` contains the frozen
...
python3 -B verify_combined.py \
  --root /path/to/exp012-working-copy \
...
fixed paths under `/content/exp012_check` and refuse existing work directories;
...
The uniqueness milestone is checked through its Lean source and exact
...
The initial fresh-VM allocation reached Colab's active-session limit. The
completed Experiment 011 VM was released after explicit approval and a
read-only preflight verified its completed proof, archived evidence and absence
of active Lean/Lake jobs. `remote_check/VM_REPLACEMENT.json` records this;
Experiment 012's accepted environment is a distinct new allocation.
