...
def validate_fresh_bootstrap(files, root, final_inputs, final_result):
    ...
    remote_root = PurePosixPath('/content/exp012_check')
    ...
    require(sha(predecessor_environment_path.read_bytes()) ==
        '915f9cd8cfa7e048cef587b053bafe84f8d389addcfb4cf0ab83f504fcd5035b',
        'Accepted Experiment 011 environment record changed')
    ...
    require(environment.get('session') == allocation.get('session') == 'exp012-independent-check'
    ...
    require(isinstance(prior_vm_ids, list) and len(prior_vm_ids) == len(set(prior_vm_ids))
        and {'m-s-kkb-usw3b1-3oqo9i5fec2ow','m-s-kkb-use1c1-1e742c6fpiifs','m-s-kkb-usc1a1-d4w1b6hb9jym',
             'm-s-kkb-usc1b1-2yftnii3g1zpd',
             'm-s-kkb-usc1c1-1p1nzzki4492q'} <= set(prior_vm_ids)
    ...
    require(isinstance(boot_id, str) and str(uuid.UUID(boot_id)) == boot_id and
        boot_id != predecessor_environment['boot_id'], 'Fresh boot identity did not change')
    ...
    require(isinstance(launch_command, list) and len(launch_command) == 4 and
        ...
        and launch_command[1:] == ['-u', '-B', '/content/exp012_bootstrap.py'] and
...
require(remote['command']==['/content/exp012_check/lean-4.31.0-linux/bin/lean','-j','1',
    '/content/exp012_check/final_source/lean/Exp012Combined.lean'],'Unexpected remote command')
require(remote['lean_path']=='/content/exp012_check/final_verification/dependencies:'
    '/content/exp012_check/lean-4.31.0-linux/lib/lean','Unexpected remote import path')
