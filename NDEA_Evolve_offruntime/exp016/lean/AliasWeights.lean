...
theorem gridAlias_frequencyWeight_le (p M : ℕ) (ell : ℤ) (r : Fin (2 * M + 1))
  ...
  have hw : Exp014.weight (oddFrequency M r) ≤ Exp014.weight ell := by
    ...
    exact pow_le_pow_left₀ (by positivity)
      (show 1 + |(oddFrequency M r : ℝ)| ≤ 1 + |(ell : ℝ)| by
        linarith [gridAlias_abs_le M ell r hr]) 2
...
theorem fourierWeightedNorm_nonneg (p M : ℕ) (h : ℝ) (y : Vec (Grid (2 * M))) :
  ...
  Finset.sum_nonneg fun r _ =>
    mul_nonneg (frequencyWeight_nonneg p (oddFrequency M r)) (norm_nonneg _)
...
theorem fourierWeightedNorm_modeLift_eq (p M : ℕ) (h : ℝ)
  ...
  · rw [fourierCoefficient_modeLift_alias M h hmesh, if_pos hr]
  · intro s _ hsr
    have hd : ¬ ((2 * M + 1 : ℕ) : ℤ) ∣ ell - oddFrequency M s :=
      fun hs => hsr ((existsUnique_gridAlias M ell).unique hs hr)
    rw [fourierCoefficient_modeLift_alias M h hmesh, if_neg hd, norm_zero, mul_zero]
  · intro hn
    exact (hn (Finset.mem_univ r)).elim
