# Exp016 continuation after actual initialization bounds
...
Read current TASK_STATE source receipts;
...
2. Prove concrete sampled-block/operator and potential-perturbation bounds with
...
The next experiment must derive accumulated spatial-defect vanishing along actual refinements, using the proved exact-sampling initialization bound. Full-band M*h<=1 is false on the chosen grid family; old small-band estimates cannot be applied wholesale. Uniform sampled-potential/operator bounds and any needed stronger graph regularity remain obligations. The sampled initial physical norm is now bounded by the existing weighted data norm; this does not control evolved derivatives. Exp016 does not assume residual-to-zero as a shortcut to the convergence theorem.
