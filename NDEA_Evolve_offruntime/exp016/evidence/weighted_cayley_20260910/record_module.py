"""Validate finished weighted Cayley modules against the frozen continuation state."""
import datetime
import hashlib
...
rp = Path(sys.argv[2])
r = read(rp)
expected_counts = {'WeightedGridStages': 13, 'WeightedCayleyPropagation': 10}
...
expected_warnings = ([
    '`ContinuousLinearMap.sub_apply` has been deprecated: Use `sub_apply` instead']
    if name == 'WeightedGridStages' else [
...
if name == 'WeightedCayleyPropagation':
    previous = read(HERE / 'WeightedGridStages_ACCEPTED.json')
...
report = {
    ...
    'warning_review': ({
        ...
        'pinned_mathlib_source': 'Mathlib/Topology/Algebra/Module/ContinuousLinearMap/Basic.lean:887',
        'declaration': '@[deprecated (since := "2026-05-20")] protected alias sub_apply := _root_.sub_apply',
        'proof_source_changed_after_check': False,
    } if name == 'WeightedGridStages' else {
        'classification': 'unnecessarySeqFocus tactic-style linter; the completed proof is accepted.',
        'source_location': str(source) + ':146',
        'reviewed_tactic': 'congr 2 <;> ring',
        'proof_source_changed_after_check': False,
    }),
...
s['recovery']['current_check'] = {
    ...
    'pending_module': 'WeightedCayleyPropagation' if name == 'WeightedGridStages' else None,
}
s['continuation']['current_status'] = str(len(s['accepted_active_modules'])) + ' accepted development modules. ' + (
    'Actual A-stage weighted preservation and B operator bound passed; trajectory propagation is next.'
    if name == 'WeightedGridStages' else
    'Actual weighted Cayley propagation and eventual schedule restrictions passed. Uniform initial moments and spatial refinement remain open. Exp016 remains unsealed.')
