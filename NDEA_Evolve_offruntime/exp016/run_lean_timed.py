#!/usr/bin/env python3
import datetime, fcntl, hashlib, json, os, pathlib, resource, subprocess, sys, time
...
env=dict(os.environ)
env['LEAN_PATH']=':'.join([str(root/'build/lib/lean'),str(root.parent/'exp015/build/lib/lean'),str(root.parent/'exp014/build/lib/lean'),str(root.parent/'exp013/build/lib/lean'),str(root.parent/'exp012/build/lib/lean'),str(root.parent/'exp011/build/lib/lean'),str(root.parent/'exp010/build/lib/lean'),str(root.parent/'exp009/build/lib/lean'),str(root.parent/'exp008/build/lib/lean')]+[str(p/'.lake/build/lib/lean') for p in packages.iterdir() if p.is_dir()]+[str(root.parent/'exp006/build/lib/lean'),str(lean.parent.parent/'lib/lean')])
if source.stem=='Exp016Combined':
...
print(json.dumps({'status':'queued', 'source':str(source), 'receipt':str(receipt)}), flush=True)
queue_begin=time.monotonic()
lock=open('/tmp/exp003_lean_one_job.lock','w')
fcntl.flock(lock,fcntl.LOCK_EX)
begin=time.monotonic()
rec['lock_wait_seconds']=begin-queue_begin
rec['compiler_started_utc']=datetime.datetime.now(datetime.timezone.utc).isoformat()
receipt.write_text(json.dumps(rec,indent=2)+'\n')
usage_before=resource.getrusage(resource.RUSAGE_CHILDREN)
print(json.dumps({'status':'compiler_running','lock_wait_seconds':rec['lock_wait_seconds'],'log':str(log)}),flush=True)
try:
...
rec['compiler_cpu_user_seconds']=usage_after.ru_utime-usage_before.ru_utime
rec['compiler_cpu_system_seconds']=usage_after.ru_stime-usage_before.ru_stime
rec['child_maxrss_kib']=usage_after.ru_maxrss
rec['child_major_faults']=usage_after.ru_majflt-usage_before.ru_majflt
rec['child_block_inputs']=usage_after.ru_inblock-usage_before.ru_inblock
