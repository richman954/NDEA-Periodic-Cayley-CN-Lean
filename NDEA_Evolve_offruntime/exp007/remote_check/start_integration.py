...
with tarfile.open('/content/exp007_integration_sources.tar.gz') as tar:
...
for name in ['SpinorGrid','ContinuumSpinor','FullClosure','StageBridge','Controls']:
 ...
 with (dev/('integration_'+name+'.log')).open('wb') as log:
 ...
 rec={'exit_code':p.returncode,'elapsed_seconds':time.monotonic()-begin,'source_sha256_before':before,'source_sha256_after':sha(s),'log_sha256':sha(dev/('integration_'+name+'.log'))}
 (dev/('integration_'+name+'_RESULT.json')).write_text(json.dumps(rec,indent=2))
 print(name,json.dumps(rec),flush=True)
 if p.returncode:break
"""
(dev/'check_integration.py').write_text(code)
with (dev/'integration_controller.log').open('wb') as log:
