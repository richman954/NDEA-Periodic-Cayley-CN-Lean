"""Re-elaborate all project source using only pinned external artifacts."""
...
record={'passed':False,'start_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),
        'runner_sha256':sha(Path(__file__)),'scope':'All project source re-elaborated; only compatible pinned external library artifacts reused.'}
...
try:
    ...
    source=root/'lean/Exp007Combined.lean'
