...
with tempfile.TemporaryDirectory(prefix='exp014_protocol_') as temp:
    fixture=Path(temp)
    ...
    shutil.copytree(root/'predecessor_sources',fixture/'predecessor_sources')
    shutil.copyfile(root/'lean/Exp013GenericFoundation.lean',fixture/'lean/Exp013GenericFoundation.lean')
    generator=load(fixture/'make_combined.py','exp014_generator_fixture')
    ...
    require('Mathlib.Analysis.InnerProductSpace.Adjoint' in inputs['external_imports'],
        'New generic Hilbert-space import was lost')
    checks += ['complete_fixture_catalog_ignores_comments','generic_adjoint_import_retained']
    source=fixture/'lean/WeightedFourier.lean';original=source.read_bytes()
    source.write_bytes(original.replace(b'import GenericUniqueness',b'import UnembeddedProject'))
    reject('unembedded_project_import_rejected',lambda:generator.build(fixture));source.write_bytes(original)
    source.write_bytes(original.replace(b'fixture_WeightedFourier',b'fixture_Controls'))
    reject('duplicate_public_name_rejected',lambda:generator.build(fixture));source.write_bytes(original)
    foundation=fixture/'lean/Exp013GenericFoundation.lean';frozen=foundation.read_bytes()
    ...
    (fixture/'lean/Exp014Combined.lean').write_text(combined)
    ...
    # fixture. This exercises the receiver schema, not Experiment 014 mathematics.
    previous=root.parent/'exp013/remote_check/downloaded_evidence'
    files={str(p.relative_to(previous)):p.read_bytes().replace(b'/content/exp013_check',b'/content/exp014_check')
        .replace(b'/content/exp013_bootstrap.py',b'/content/exp014_bootstrap.py')
        for p in previous.rglob('*') if p.is_file()}
    environment=json.loads(files['ENVIRONMENT.json'])
    environment.update(session='exp014-independent-check',boot_id='00000000-0000-4000-8000-000000000014')
    ...
    allocation={'utc':environment['utc'],'root':'/content/exp014_check','session':'exp014-independent-check',
        'vm_id':'SYNTHETIC_EXP013_FIXTURE','distinct_allocation':True,'previous_vm_ids':
        ['m-s-kkb-usw4c1-1loxz56gsupls','m-s-kkb-usw3b1-3oqo9i5fec2ow','m-s-kkb-use1c1-1e742c6fpiifs','m-s-kkb-usc1a1-d4w1b6hb9jym','m-s-kkb-usc1b1-2yftnii3g1zpd','m-s-kkb-usc1c1-1p1nzzki4492q','m-s-kkb-usc1c0-1k0px308tu45q','m-s-kkb-use1c2-3vyfjmsvu7xz8']}
    ...
    bootstrap['cache_modules']=sorted(m for m in inputs['external_imports'] if m.startswith('Mathlib.'))
    ...
    for row in bootstrap['commands']:
        key=str(PurePosixPath(row['log']).relative_to('/content/exp014_check'))
    ...
    mutation('incomplete_cache_roots_rejected','bootstrap/RESULT.json',
        lambda x:x['cache_modules'].remove('Mathlib.Analysis.InnerProductSpace.Adjoint'))
    ...
    allocation['vm_id']=allocation['previous_vm_ids'][-1]
    write_json(fixture/'remote_check/VM_ALLOCATION.json',allocation)
...
record={'passed':True,'utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),
    ...
    'scope':'Bounded offline infrastructure exercises with synthetic and transformed predecessor fixtures. No Experiment 014 Lean proof or network action is performed or accepted by these tests.'}
