"""Bounded offline tests for the generic-only Exp014 combined source protocol."""
import ast, datetime, hashlib, importlib.util, json, shutil, tempfile
from pathlib import Path

root = Path(__file__).resolve().parent
spec = importlib.util.spec_from_file_location('exp014_generator_test', root/'make_combined.py')
...
accepted('exact_generic_foundation_reconstruction',
         foundation == (root/'lean/Exp013GenericFoundation.lean').read_text())
accepted('generic_provenance_matches_sealed_originals',
         all((root/path).read_bytes() == (root.parent/'exp013/lean'/Path(path).name).read_bytes()
             for path in pins))
with tempfile.TemporaryDirectory(prefix='exp014_combined_fixture_') as temporary:
    ...
    for relative in list(pins)+['verification_tools/verify_exp005.py',
                                'lean/Exp013GenericFoundation.lean']:
    ...
    for index, name in enumerate(generator.MODULES):
        ...
        expected.append('NDEAEvolve.Exp014.public_'+name)
    ...
    accepted('only_generic_predecessor_bodies_are_embedded',
             'NDEAEvolve.Exp012' not in combined and 'NDEAEvolve.Exp011' not in combined
             and 'namespace NDEAEvolve.Exp013' in combined
             and 'theorem classical_unique' in combined)
    ...
    first = fixture/'predecessor_sources/GenericClassical.lean'
    original = first.read_bytes()
    first.write_bytes(original+b'\n')
    rejected('changed_generic_original_rejected', lambda: generator.build(fixture))
    first.write_bytes(original)
    first = fixture/'lean/Exp013GenericFoundation.lean'
    original = first.read_bytes()
    first.write_bytes(original+b'\n')
    ...
    control.write_text(original.replace('public_Controls', 'public_StrongOperatorDerivative'))
