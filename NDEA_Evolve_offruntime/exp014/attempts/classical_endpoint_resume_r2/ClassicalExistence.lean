...
theorem solution_continuous (v : ℤ → H →L[ℂ] H) (b₀ : FourierState H) :
    Continuous (fun p : ℝ × ℝ => solution v b₀ p.1 p.2) := by
  change Continuous (fun p : ℝ × ℝ => synth (physicalState v b₀ p.1) p.2)
  exact (synth_joint_continuous (H := H)).comp
    (continuous_snd.prodMk ((physicalState_continuous v b₀).comp continuous_fst))
...
theorem solution_classical (v : ℤ → H →L[ℂ] H) (hv : RegularPotential v)
  ...
  continuous_time_derivative := by
    have hsecond : Continuous (fun p : ℝ × ℝ => synthSecond (physicalState v b₀ p.1) p.2) :=
      (synthSecond_joint_continuous (H := H)).comp
        (continuous_snd.prodMk ((physicalState_continuous v b₀).comp continuous_fst))
    have hvu := ((operatorPotential_continuous v hv).comp continuous_snd).clm_apply
      (solution_continuous v b₀)
    have h := (hsecond.const_smul Complex.I).add (hvu.const_smul (-Complex.I))
    convert h using 1
    funext p
    exact (solution_time_hasDerivAt v hv b₀ p.1 p.2).deriv
  continuous_space_derivative := by
    have h := (synthFirst_joint_continuous (H := H)).comp
      (continuous_snd.prodMk ((physicalState_continuous v b₀).comp continuous_fst))
    convert h using 1
    funext p
    exact (solution_space_hasDerivAt v b₀ p.1 p.2).deriv
  continuous_second_derivative := by
    have h := (synthSecond_joint_continuous (H := H)).comp
      (continuous_snd.prodMk ((physicalState_continuous v b₀).comp continuous_fst))
    convert h using 1
    funext p
    exact (solution_second_hasDerivAt v b₀ p.1 p.2).deriv
  ...
  schrodinger t x := by
    ...
    simp only [Pi.zero_apply, add_zero, smul_add, smul_smul]
    simp [Complex.I_sq, ← neg_mul, ← mul_neg, neg_smul]
