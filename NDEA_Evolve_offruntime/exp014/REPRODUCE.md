...
The independent qualification requires a new CPU VM with no preexisting project
...
The combined source in `lean/Exp014Combined.lean` contains the frozen
three generic Experiment 013 proof bodies followed by all nine new modules and the complete new
...
python3 -B verify_combined.py \
  --root /path/to/exp014-working-copy \
...
only the three generic predecessor bodies retained in the reconstructed
foundation. The exact originals are preserved under predecessor_sources/.
...
fixed paths under `/content/exp014_check` and refuse existing work directories;
...
The global classical existence and uniqueness milestone is checked through its
Lean source and exact controls. No floating-point calculation substitutes for
the infinite-series existence argument. Individual
...
The new session is `exp014-independent-check`; initial environment and VM
