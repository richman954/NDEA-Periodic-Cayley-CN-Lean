...
def validate_fresh_bootstrap(files, root, final_inputs, final_result):
    ...
    remote_root = PurePosixPath('/content/exp014_check')
    ...
    require(sha(predecessor_environment_path.read_bytes()) ==
        'de143ee6d8202858c60d4524762a441aa069409d1e1ecec938d71487d5cfd714',
        'Accepted Experiment 013 environment record changed')
    ...
    require(environment.get('session') == allocation.get('session') == 'exp014-independent-check'
    ...
    require(isinstance(prior_vm_ids, list) and len(prior_vm_ids) == len(set(prior_vm_ids))
        and {'m-s-kkb-usw4c1-1loxz56gsupls','m-s-kkb-usw3b1-3oqo9i5fec2ow','m-s-kkb-use1c1-1e742c6fpiifs','m-s-kkb-usc1a1-d4w1b6hb9jym',
             ...
             'm-s-kkb-use1c2-3vyfjmsvu7xz8'} <= set(prior_vm_ids)
    ...
    require(isinstance(launch_command, list) and len(launch_command) == 4 and
        ...
        and launch_command[1:] == ['-u', '-B', '/content/exp014_bootstrap.py'] and
...
require(remote['command']==['/content/exp014_check/lean-4.31.0-linux/bin/lean','-j','1',
    '/content/exp014_check/final_source/lean/Exp014Combined.lean'],'Unexpected remote command')
require(remote['lean_path']=='/content/exp014_check/final_verification/dependencies:'
    '/content/exp014_check/lean-4.31.0-linux/lib/lean','Unexpected remote import path')
