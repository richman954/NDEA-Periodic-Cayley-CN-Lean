...
record={'passed':False,'start_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),
        'runner_sha256':sha(Path(__file__)),'scope':'Experiment 014 sources and retained Experiment 013 generic PDE sources re-elaborated using compatible external artifacts from pinned source checkouts, with recorded hashes. Earlier numerical proof chains are outside this combined source. Compiler and library artifacts remain trusted inputs; this check does not rebuild them or prove source-to-artifact correspondence.'}
...
try:
    ...
    source=root/'lean/Exp014Combined.lean'
    ...
    project_modules={Path(name).stem for name in inputs['source_hashes']}|{'Exp014Combined','GenericClassical','GenericEnergy','GenericUniqueness'}
