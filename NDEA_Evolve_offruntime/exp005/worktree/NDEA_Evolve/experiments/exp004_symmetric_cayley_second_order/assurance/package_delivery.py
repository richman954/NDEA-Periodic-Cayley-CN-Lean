#!/usr/bin/env python3
"""Package an already committed Experiment 004 release and verify recovery locally.
...
EXPERIMENT = "experiments/exp004_symmetric_cayley_second_order"
FINAL_MANIFEST = f"{EXPERIMENT}/evidence/FINAL_SHA256SUMS"
DEFAULT_RELEASE = Path("/home/richman954/NDEA_Evolve_offruntime/exp004/releases/20260907_verified_final")
ARCHIVE_PREFIX = "NDEA_Evolve/"
PREDECESSOR_TAG = "exp003-step5-continuous-exponential-comparison-verified-final-20260907"
PREDECESSOR_COMMIT = "3a5078a02bbc1423a302016d4e14c78f874f021f"
...
class Delivery:
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        self.archive = self.release / "exp004_verified_final_source.tar.gz"
        self.bundle = self.release / "exp004_verified_final.bundle"
        self.receipt = self.release / "DELIVERY_RECEIPT.json"
        self.release_created = False
        self.record: dict = {
            "schema": "ndea.exp004.delivery.v1",
    ...
    def execute(self) -> None:
        ...
        require(predecessor == PREDECESSOR_COMMIT,
                "Source Step-5 predecessor tag does not resolve to the expected release commit.")
        ...
        self.record["bundle_reference_scope"] = "All refs in the isolated Experiment 004 release repository, including predecessor release tags."
        ...
        temporary = Path(self.run("create_fresh_recovery_directory",
            ["mktemp", "-d", "/tmp/exp004_delivery_verify.XXXXXX"]).strip())
        require(temporary.is_dir() and temporary.parent == Path("/tmp")
                and temporary.name.startswith("exp004_delivery_verify."),
        ...
        require(recovered_predecessor == PREDECESSOR_COMMIT,
                "Recovered bundle did not preserve the pinned Step-5 release tag.")
