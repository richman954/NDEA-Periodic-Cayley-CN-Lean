#!/usr/bin/env python3
"""Audit explicitly selected final Experiment 004 evidence; never run Lean or overwrite audits."""
...
PREDECESSOR = "3a5078a02bbc1423a302016d4e14c78f874f021f"
PREDECESSOR_TAG = "exp003-step5-continuous-exponential-comparison-verified-final-20260907"
...
REQUIRED_ENDPOINTS = {
    "NDEAEvolve.Exp004.symmetric_cayley_exp_local_opNorm_le",
    "NDEAEvolve.Exp004.symmetric_cayley_fixed_time_error_le",
    "NDEAEvolve.Exp004.symmetric_cayley_fixed_time_tendsto_exp",
}
...
class Audit:
    ...
    def predecessor(self) -> None:
        ...
        baseline_path = Path(__file__).with_name("PREDECESSOR_BASELINE_20260907T234733Z.json").resolve()
        ...
        self.check(head.returncode == 0 and head.stdout.strip() == PREDECESSOR,
                   "Original Step-5 worktree HEAD changed.")
        self.check(tag.returncode == 0 and tag.stdout.strip() == PREDECESSOR,
                   "Original Step-5 tag changed.")
        self.check(original_tag_object.returncode == 0 and inherited_tag_object.returncode == 0
                   ...
                   "Original and inherited Step-5 raw tag objects differ.")
        self.check(status.returncode == 0 and status.stdout == baseline["status_porcelain_v1"] == "",
                   "Original Step-5 worktree is no longer clean.")
        ...
        self.report["original_predecessor_and_release_check"] = {
            ...
            "baseline_captured_utc": baseline["captured_during_exp004_utc"],
        ...
        self.check(release_hashes == baseline["release_sha256"], "Original Step-5 release archives changed.")
    ...
    def policy(self, sources: list[Path]) -> list[str]:
        ...
        for path in sources:
            ...
            if path.name == "NegativeControls.lean":
                controls = PUBLIC_THEOREM.findall(code)
                audited_basenames = {name.rsplit(".", 1)[-1] for name in source_axioms}
                self.check(bool(controls), "Controls file has no named theorem witnesses.")
                for control in controls:
                    self.check(control.rsplit(".", 1)[-1] in audited_basenames,
                               f"Named control theorem lacks #print axioms: {control}")
                self.report["named_control_theorems"] = controls
