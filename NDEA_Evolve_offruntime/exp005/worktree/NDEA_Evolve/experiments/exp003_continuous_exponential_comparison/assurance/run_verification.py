...
class Runner:
    ...
    def run(self, label: str, command: list[str], sources: list[Path], outputs: list[Path] | None = None) -> None:
        ...
        record = {
            ...
            "label": label,
            "qualification": "development" if self.args.skip_existing else "fresh",
            "command": command,
            "cwd": str(self.repo),
    ...
    def combined(self, controls: bool) -> None:
        ...
        target = self.exp / "evidence/generated" / f"CombinedStep5{'Controls' if controls else 'Production'}_{self.run_id}.lean"
...
def main() -> None:
    ...
    parser.add_argument("stage", choices=("prepare", "predecessors", "module", "probe", "production", "controls", "combined", "combined-controls"))
    parser.add_argument("--repo", type=Path, default=Path(__file__).resolve().parents[3])
    parser.add_argument("--cache-root", type=Path, default=Path("/tmp/exp003_step5_build"))
