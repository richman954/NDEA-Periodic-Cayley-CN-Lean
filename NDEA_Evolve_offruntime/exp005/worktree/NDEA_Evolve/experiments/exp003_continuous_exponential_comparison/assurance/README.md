...
python3 run_verification.py module --path /absolute/path/to/ExponentialRemainder.lean
...
`/tmp/exp003_step5_build/lib/lean`; it does not run Lean or download dependencies.
...
root. `production` and `controls` compile all predecessors and Step-5 modules
...
single fresh source containing the entire predecessor and Step-5 chain, with
...
predecessor file against Step 4, narrow-import shadow provenance, source policy,
...
earlier release tags. Recovery also verifies that the pinned Step-4 tag resolves
to `902a5d44c222254a443a01aebf563df72b8154d5`.
