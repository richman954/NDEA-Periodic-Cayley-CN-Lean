...
function cayley_data(H::Matrix{G}, a::Q; require_hermitian::Bool=true)
    ...
    for label in structural_labels
        assert_zero(label, residuals[label])
    end
    if require_hermitian
        for label in (
            ...
            "D_dagger_minus_N",
            "N_dagger_minus_D",
            "Ddagger_D_minus_gram",
            "Ndagger_N_minus_gram",
            "Udagger_U_minus_I",
            "U_Udagger_minus_I",
        )
            assert_zero(label, residuals[label])
...
semigroup_one_record, semigroup_one_raw = cayley_data(Hscalar, q(1))
semigroup_two_record, semigroup_two_raw = cayley_data(Hscalar, q(2))
semigroup_product = mat_mul(semigroup_one_raw.U, semigroup_one_raw.U)
semigroup_merge_residual = mat_sub(semigroup_product, semigroup_two_raw.U)
is_zero_matrix(semigroup_merge_residual) && error("false Cayley semigroup merging unexpectedly passed")
...
reversed_equivalent_rhs = mat_scale(gq(-4 * q(1, 2) * q(2, 3)),
    mat_mul(mat_mul(mat_mul(mat_mul(raw2.Dinv, raw1.Dinv), generator_commutator), raw1.Dinv), raw2.Dinv))
assert_zero("fully reversed equivalent order-defect factorization",
            mat_sub(cayley_commutator, reversed_equivalent_rhs))
wrong_left_order_rhs = mat_scale(gq(-4 * q(1, 2) * q(2, 3)),
    mat_mul(mat_mul(mat_mul(mat_mul(raw2.Dinv, raw1.Dinv), generator_commutator), raw2.Dinv), raw1.Dinv))
wrong_left_order_residual = mat_sub(cayley_commutator, wrong_left_order_rhs)
is_zero_matrix(wrong_left_order_residual) && error("one-sided inverse permutation unexpectedly passed")

# Exhaustive lexicographic search over all 2x2 real matrices with entries {-1,0,1}.
function exhaustive_counterexample_search()
    search_total = 0
    search_singular = 0
    search_invertible = 0
        ...
        if !is_hermitian_exact(H)
            ...
            if !is_zero_matrix(defect)
                ...
                if first_nonhermitian_nonunitary === nothing
                    first_nonhermitian_nonunitary = Dict(
                        "H" => matrix_json(H), "D" => matrix_json(D),
    ...
    return (
        total=search_total,
        singular=search_singular,
        invertible=search_invertible,
...
search = exhaustive_counterexample_search()
search_total = search.total
search_singular = search.singular
search_invertible = search.invertible
search_nonhermitian_invertible = search.nonhermitian_invertible
search_nonhermitian_nonunitary = search.nonhermitian_nonunitary
first_singular = search.first_singular
first_nonhermitian_nonunitary = search.first_nonhermitian_nonunitary
...
core = Dict(
    ...
    "order_defect" => Dict(
        ...
        "equivalent_fully_reversed_rhs" => matrix_json(reversed_equivalent_rhs),
        "residual" => matrix_json(mat_sub(cayley_commutator, order_rhs)),
        "fully_reversed_residual" => matrix_json(mat_sub(cayley_commutator, reversed_equivalent_rhs)),
    ...
    "adversarial_witnesses" => [
        ...
        Dict("id" => "false_cayley_semigroup_merging", "classification" => "EXACT_COUNTEREXAMPLE",
             "generator" => matrix_json(Hscalar), "a" => rational_json(q(1)), "b" => rational_json(q(1)),
             "C_a" => matrix_json(semigroup_one_raw.U), "C_b" => matrix_json(semigroup_one_raw.U),
             "C_a_times_C_b" => matrix_json(semigroup_product),
             "C_a_plus_b" => matrix_json(semigroup_two_raw.U),
             "residual" => matrix_json(semigroup_merge_residual)),
        ...
        Dict("id" => "wrong_left_inverse_order", "classification" => "EXACT_COUNTEREXAMPLE",
             "description" => "left inverse pair swapped while right pair is unchanged",
             "wrong_rhs" => matrix_json(wrong_left_order_rhs), "residual" => matrix_json(wrong_left_order_residual)),
...
numeric_trials = 512
numeric_dimensions = [2, 3, 4]
numeric_factors_per_product = 3
numeric_acceptance_limit = 1.0e-10
function numerical_experiments(trials, dimensions, factors_per_product)
    ...
    for _ in 1:trials
        for n in dimensions
            ...
            for _ in 1:factors_per_product
                ...
                N = identity - im * a * H
                U = D \ N
                max_single_residual = max(max_single_residual, opnorm(U' * U - identity, 2))
                push!(factors, U)
            end
            P = factors[3] * factors[2] * factors[1]
            max_product_residual = max(max_product_residual, opnorm(P' * P - identity, 2))
        end
    end
    return max_single_residual, max_product_residual
end
max_single_residual, max_product_residual = numerical_experiments(
    numeric_trials, numeric_dimensions, numeric_factors_per_product)
numeric_pass = isfinite(max_single_residual) && isfinite(max_product_residual) &&
               max_single_residual <= numeric_acceptance_limit &&
               max_product_residual <= numeric_acceptance_limit
numeric_pass || error("numeric residual exceeded independently fixed acceptance limit")
...
receipt = Dict(
    ...
    "numeric_nonproof" => Dict(
        ...
        "seed" => 20260905,
        "trials_per_dimension" => numeric_trials,
        "matrix_dimensions" => numeric_dimensions,
        "factors_per_product" => numeric_factors_per_product,
        "single_factor_checks" => numeric_trials * length(numeric_dimensions) * numeric_factors_per_product,
        "ordered_product_checks" => numeric_trials * length(numeric_dimensions),
        "residual_norm" => "Euclidean induced operator 2-norm via LinearAlgebra.opnorm(A, 2)",
        "acceptance_limit" => string(numeric_acceptance_limit),
        "acceptance_limit_provenance" => "independently fixed in source before execution; not certificate-controlled",
    ...
    "exact_status" => "PASS",
    "numeric_status" => numeric_pass ? "PASS" : "FAIL",
)
...
println("ADVERSARIAL_WITNESSES=11/11 PASS")
println("NUMERIC_TRIALS_PER_DIMENSION=", numeric_trials)
println("NUMERIC_DIMENSIONS=", join(numeric_dimensions, ","))
println("NUMERIC_SINGLE_FACTOR_CHECKS=", numeric_trials * length(numeric_dimensions) * numeric_factors_per_product)
println("NUMERIC_ORDERED_PRODUCT_CHECKS=", numeric_trials * length(numeric_dimensions))
println("NUMERIC_RESIDUAL_NORM=EUCLIDEAN_INDUCED_OPERATOR_2_NORM")
println("NUMERIC_ACCEPTANCE_LIMIT=", numeric_acceptance_limit)
