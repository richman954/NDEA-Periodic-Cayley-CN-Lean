#!/usr/bin/env python3
"""Package an already committed Experiment 005 release and verify recovery locally.
...
EXPERIMENT = "experiments/exp005_mesh_weighted_residual_bridge"
FINAL_MANIFEST = f"{EXPERIMENT}/evidence/FINAL_SHA256SUMS"
DEFAULT_RELEASE = Path("/home/richman954/NDEA_Evolve_offruntime/exp005/releases/20260907_verified_final")
ARCHIVE_PREFIX = "NDEA_Evolve/"
PREDECESSOR_TAG = "exp004-symmetric-cayley-second-order-verified-final-20260907"
PREDECESSOR_COMMIT = "956ce8acac594c45cf576b66972a082c89190bf8"
...
class Delivery:
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        self.archive = self.release / "exp005_verified_final_source.tar.gz"
        self.bundle = self.release / "exp005_verified_final.bundle"
        self.receipt = self.release / "DELIVERY_RECEIPT.json"
        self.release_created = False
        self.record: dict = {
            "schema": "ndea.exp005.delivery.v1",
    ...
    def execute(self) -> None:
        ...
        require(predecessor == PREDECESSOR_COMMIT,
                "Source Experiment 004 predecessor tag does not resolve to the expected release commit.")
        ...
        self.record["bundle_reference_scope"] = "All refs in the isolated Experiment 005 release repository, including predecessor release tags."
        ...
        temporary = Path(self.run("create_fresh_recovery_directory",
            ["mktemp", "-d", "/tmp/exp005_delivery_verify.XXXXXX"]).strip())
        require(temporary.is_dir() and temporary.parent == Path("/tmp")
                and temporary.name.startswith("exp005_delivery_verify."),
        ...
        require(recovered_predecessor == PREDECESSOR_COMMIT,
                "Recovered bundle did not preserve the pinned Experiment 004 release tag.")
