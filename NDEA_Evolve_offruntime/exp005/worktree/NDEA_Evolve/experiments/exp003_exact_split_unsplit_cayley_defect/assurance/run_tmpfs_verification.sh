...
stage=${1:-production}
repo=/home/richman954/NDEA_Evolve_offruntime/exp003_step3/worktree/NDEA_Evolve
...
step3="$repo/experiments/exp003_exact_split_unsplit_cayley_defect"
shadow="$step2/proof_attempts/shadow_sources"
ram=/dev/shm/exp003_step3_shadow
...
if [[ "$stage" == combined ]]; then
  combined="$ram/CombinedStep3Verification.lean"
  python3 "$step3/assurance/make_combined_verification.py" \
...
timeout 900s lean -j 1 -R "$repo" \
  "$step3/controls/lean/NegativeControls.lean"
