...
def validate_fresh_bootstrap(files, root, final_inputs, final_result):
    ...
    remote_root = PurePosixPath('/content/exp010_check')
    ...
    require(sha(predecessor_environment_path.read_bytes()) ==
        'ee6dff6af0e5df4567462a2e5bfd91304bde7e68921b8b670eab1589480afe03',
        'Accepted Experiment 009 environment record changed')
    ...
    require(environment.get('session') == allocation.get('session') == 'exp010-independent-check'
    ...
    require(isinstance(prior_vm_ids, list) and len(prior_vm_ids) == len(set(prior_vm_ids))
        and {'m-s-kkb-usc1a1-d4w1b6hb9jym','m-s-kkb-usc1b1-2yftnii3g1zpd',
    ...
    require(isinstance(launch_command, list) and len(launch_command) == 4 and
        ...
        and launch_command[1:] == ['-u', '-B', '/content/exp010_bootstrap.py'] and
...
require(remote['command']==['/content/exp010_check/lean-4.31.0-linux/bin/lean','-j','1',
    '/content/exp010_check/final_source/lean/Exp010Combined.lean'],'Unexpected remote command')
require(remote['lean_path']=='/content/exp010_check/final_verification/dependencies:'
    '/content/exp010_check/lean-4.31.0-linux/lib/lean','Unexpected remote import path')
