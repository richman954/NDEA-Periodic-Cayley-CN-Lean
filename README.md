...
Exp016 is authorized and in progress. This checkpoint contains **37 accepted development modules**: 35 property modules with 224 public theorem declarations, a separate norm axiom-audit module, and independently authored target definitions. Pending sources: none at this capture. The [current alias and tail catalog](NDEA_Evolve_offruntime/exp016/ALIAS_CHECKPOINT.json) binds every captured mathematical source to its completed receipt and full compiler log. **Exp016 remains incomplete and unsealed:** no combined or independent Exp016 qualification is included.

The new grid-alias identities apply to arbitrary integer frequencies on the full odd grid. The sampled-potential coefficient formula includes every integer shift by the grid size. The actual potential interpolation defect is expanded into its mode discrepancies, including both interpolated aliases and the high frequencies of the continuum product. The resulting cutoff bound is `2 * sqrt(2*pi) * (potential_tail(M-R) * state_low_l1(R) + potential_total_l1 * state_tail(R))`, with inclusive low-frequency cutoffs and their strict complementary tails. These are the actual current Fourier coefficients and the original regular-potential assumptions; no uniform numerical tail bound is assumed or proved.

The alias-tail certificate substitutes this derived potential bound and the accepted stencil bound into the actual ordered Cayley trajectory estimates, at grid times and within each partial slab, retaining arbitrary initial error. Exact support controls prove equality of the sampled reconstruction and continuum product when the state and potential bandwidths satisfy `R + K <= M`. The actual five-node cosine-potential and nonzero +1-mode example includes the boundary frequency +2. These support controls have their own stated hypotheses; they do not assert that general numerical trajectories remain supported in a fixed band.

The earlier accepted ordered-stage identities and norm bounds remain unchanged. Their operator norms are those of the actual grid matrices; no bound uniform across mesh refinements is asserted. The full-grid stencil bound has factor `sqrt(2*pi) * h^2` times the actual fourth-frequency moment of the current grid coefficients. No fourth-moment bound uniform along the numerical trajectory or across refinements is supplied. The current tail and stencil estimates therefore do not establish spatial-defect vanishing, a refinement limit, or a general variable-potential convergence rate.

Weighted coefficient extraction identifies the continuum Fourier coefficients of Exp014's all-mode synthesis and its potential product with their integer convolution. The sampled alias formula now supplies the separate discrete coefficient identity. The actual sampled-matrix potential is bound to Exp014's operator-valued potential, and the independently specified target is connected to the reconstructed trajectory by accepted theorems. An independently written target definition alone is not a PDE property theorem. Earlier finite-reconstruction Parseval and coefficient identities retain their accepted scope.
...
## Continue from this alias checkpoint

Read the [startup milestone report](NDEA_Evolve_offruntime/exp016/STARTUP_MILESTONE.md), [NEXT_STEPS](NDEA_Evolve_offruntime/exp016/NEXT_STEPS.md) and the [regular synthesis interface](NDEA_Evolve_offruntime/exp016/design/REGULAR_SYNTHESIS_INTERFACE.md) for the immediate bridge to the actual PDE residual. The [density/perturbation option](NDEA_Evolve_offruntime/exp016/design/DENSITY_PERTURBATION_OPTION.md) is a later mathematical design possibility, not a new accepted theorem or an Exp017 selection. The current source snapshot preserves the exact acceptance boundary in ALIAS_CHECKPOINT.json even when later workspace checks finish. Earlier startup, certificate, transfer and defects catalogs retain their historical meaning.
