...
Exp016 is authorized and in progress. This snapshot includes **5 locally accepted modules, 39 theorem declarations total**: `AffineReconstruction.lean` (14 declarations), `FourierReconstruction.lean` (7 declarations), `QuadraticTime.lean` (10 declarations), `SpatialL2Bridge.lean` (4 declarations), `SplitDefect.lean` (4 declarations). Exact completed receipts and compiler logs are bound in the [acceptance catalog](NDEA_Evolve_offruntime/exp016/MODULAR_ACCEPTANCE.json). **Exp016 remains incomplete and unsealed:** no combined check/axiom audit or independent Exp016 qualification is included. Earlier failed receipts remain historical diagnostics.

Start with [Exp016 PLAN.md](NDEA_Evolve_offruntime/exp016/PLAN.md), the [snapshot status and exact source hashes](NDEA_Evolve_offruntime/exp016/DEVELOPMENT_SNAPSHOT.json), and the [existing numerical API audit](NDEA_Evolve_offruntime/exp016/design/EXISTING_NUMERICAL_API.md). The proposed endpoint is a faithful full-grid Fourier reconstruction, a quadratic extension on each time slab, and an actual ordered-Cayley residual certificate against Exp014 solutions using Exp015. Spatial refinement and broader variable-potential PDE convergence remain open. The root [development plan](DEVELOPMENT_PLAN.md) gives the scope transition.

Exp015 remains the unchanged accepted predecessor: its [completion report](NDEA_Evolve_offruntime/exp015/COMPLETION_REPORT.md) records local and independent qualification and sealing. Exp013 establishes generic classical energy and uniqueness; Exp014 constructs classical solutions for a weighted Fourier variable-potential class; Exp015 proves continuous L2 forcing/residual estimates with coefficient 1 and arbitrary initial error. The accepted startup foundations supply ordered split-defect algebra, affine regularity/residual identities, spatial L2 scaling/triangle rules, and the additional exact identities listed in the catalog. They do not establish the complete Fourier L2 isometry, a vanishing spatial residual, or a variable-potential convergence rate.

## Layout and evidence

`NDEA_Evolve_offruntime/exp013` through `exp015` retain exact accepted Lean sources, combined sources, verification/audit Python, controls, tests, dependency pins, derivations and compact receipts. Exp016 adds only the explicitly selected startup-foundations snapshot and an exact Exp015 combined-source copy. Its required Exp008 `StageBridge.lean` import is retained with its original sealed manifest binding. Its `BASELINE.json` and `NUMERICAL_PINS.json` retain predecessor source/packet bindings. The selected Exp008–012 Lean files complete the actual modular import chain. Their packet manifests and receipts bind those bytes to earlier sealed packets. The older experiment folders here are deliberately partial.
...
## Continue from this foundations checkpoint

Read the [startup milestone report](NDEA_Evolve_offruntime/exp016/STARTUP_MILESTONE.md), [NEXT_STEPS](NDEA_Evolve_offruntime/exp016/NEXT_STEPS.md) and the [regular synthesis interface](NDEA_Evolve_offruntime/exp016/design/REGULAR_SYNTHESIS_INTERFACE.md) for the immediate bridge to the actual PDE residual. The [density/perturbation option](NDEA_Evolve_offruntime/exp016/design/DENSITY_PERTURBATION_OPTION.md) is a later mathematical design possibility, not a new accepted theorem or an Exp017 selection. Source snapshots preserve the exact acceptance boundary recorded in MODULAR_ACCEPTANCE.json even when later workspace checks finish.
