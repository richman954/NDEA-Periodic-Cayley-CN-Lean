...
Exp016 is authorized and in progress. This snapshot includes **two locally accepted modules**: `SplitDefect.lean` (four theorem declarations) and `AffineReconstruction.lean` (14 theorem declarations), with their exact completed receipts and compiler logs. See the [source-to-receipt binding and declaration catalog](NDEA_Evolve_offruntime/exp016/MODULAR_ACCEPTANCE.json). **Exp016 remains incomplete and unsealed:** no combined check/axiom audit or independent Exp016 qualification is included. Earlier failed receipts are retained as historical diagnostics. In-flight Fourier, quadratic and spatial-norm drafts are outside this bounded snapshot.
